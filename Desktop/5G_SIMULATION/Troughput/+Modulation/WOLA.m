classdef WOLA < handle
    % =====================================================================    
    % This MATLAB class represents an implementation of WOLA (windowed
    % OFDM). The modulation parameters are initialized by the class
    % contructor. The modulation of data symbols x and the demodulation of 
    % the received samples r is then performed by the methods 
    % ".Modulation(x)" and ".Demodulation(r)".
    ...
    methods
        function obj = WOLA(varargin)
            ...
            if numel(varargin) == 10
                ...
                % WOLA!
                obj.PHY.OverlapLengthTX         = varargin{9};      % Length of the window overlapping (s) at the transmitter 
                obj.PHY.OverlapLengthRX         = varargin{10};     % Length of the window overlapping (s) at the receiver
            elseif numel(varargin) == 0
                ...
                % WOLA!
                obj.PHY.OverlapLengthTX         = 1/2*1/(14*15e3);
                obj.PHY.OverlapLengthRX         = 1/2*1/(14*15e3);                  
            else
...
%             obj.Nr.SamplesTotal                         = (obj.Nr.MCSymbols*obj.Implementation.TimeSpacing)+2*obj.Implementation.ZeroGuardSamples;          % total number of samples for all symbols including zero guards
            ...
            % WOLA
            obj.Implementation.OverlapSamplesTX         = round(obj.PHY.OverlapLengthTX*obj.PHY.SamplingRate); % Maybe implement Warning if it does not fit
            obj.PHY.OverlapLengthTX                     = obj.Implementation.OverlapSamplesTX/obj.PHY.SamplingRate;       
            WindowFunctionTempTX                        = sqrt((cos(linspace(-pi,0,obj.Implementation.OverlapSamplesTX+2).')+1)/2);
            obj.Implementation.WindowingFunctionTX      = WindowFunctionTempTX(2:end-1);

            obj.Implementation.OverlapSamplesRX         = round(obj.PHY.OverlapLengthRX*obj.PHY.SamplingRate); % Maybe implement Warning if it does not fit
            obj.PHY.OverlapLengthRX                     = obj.Implementation.OverlapSamplesRX/obj.PHY.SamplingRate;       
            WindowFunctionTempRX                        = sqrt((cos(linspace(-pi,0,obj.Implementation.OverlapSamplesRX+2).')+1)/2);
            obj.Implementation.WindowingFunctionRX      = WindowFunctionTempRX(2:end-1);
 
            obj.Implementation.TimeSpacing              = round(obj.Implementation.FFTSize + obj.Implementation.CyclicPrefix + obj.Implementation.OverlapSamplesTX + obj.Implementation.OverlapSamplesRX); 
            obj.PHY.TimeSpacing                         = obj.Implementation.TimeSpacing*obj.PHY.dt;
            obj.Nr.SamplesTotal                         = round((obj.Nr.MCSymbols*obj.Implementation.TimeSpacing+obj.Implementation.OverlapSamplesTX)+2*obj.Implementation.ZeroGuardSamples);    
         
            
            OverlappingIndex =[ones(obj.Implementation.TimeSpacing+obj.Implementation.OverlapSamplesTX,obj.Nr.MCSymbols);zeros(obj.Implementation.TimeSpacing*(obj.Nr.MCSymbols-1),obj.Nr.MCSymbols)];
            for i_k = 1:obj.Nr.MCSymbols
                OverlappingIndex(:,i_k) = circshift(OverlappingIndex(:,i_k),(i_k-1)*obj.Implementation.TimeSpacing);
            end
            obj.Implementation.OverlappingIndex         = logical(OverlappingIndex);
        end
        ...
        function TransmitSignal = Modulation(obj,DataSymbols)
            ...
            % of size "Number of subcarriers" \times "Number of WOLA symbols (time)"
            ...
            % WOLA
            OverlapBegin = bsxfun(@times,obj.Implementation.WindowingFunctionTX,TransmitSignalNoCP((end-(obj.Implementation.CyclicPrefix+obj.Implementation.OverlapSamplesTX+obj.Implementation.OverlapSamplesRX))+1:(end-obj.Implementation.CyclicPrefix-obj.Implementation.OverlapSamplesRX),:));
            OverlapEnd = bsxfun(@times,obj.Implementation.WindowingFunctionTX(end:-1:1),TransmitSignalNoCP(1:obj.Implementation.OverlapSamplesTX,:));
            CyclicPrefixPlusOverlapRX = TransmitSignalNoCP(end-obj.Implementation.CyclicPrefix-obj.Implementation.OverlapSamplesRX+1:end,:);

            TransmitSignalNoZeroGuard = zeros(size(obj.Implementation.OverlappingIndex));
            TransmitSignalNoZeroGuard(obj.Implementation.OverlappingIndex) = [OverlapBegin;CyclicPrefixPlusOverlapRX;TransmitSignalNoCP;OverlapEnd];  
            TransmitSignal =[zeros(obj.Implementation.ZeroGuardSamples,1);sum(TransmitSignalNoZeroGuard,2);zeros(obj.Implementation.ZeroGuardSamples,1)];     
        end
        
        function ReceivedSymbols = Demodulation(obj, ReceivedSignal)
            % demodulates the received time signal and returns a matrix of 
            % size "Number of subcarriers" \times "Number of WOLA symbols"
            % which represents the received symbols after demodulation but
            % before channel equalization
            
            ReceivedSignal_reshaped = reshape(ReceivedSignal((obj.Implementation.ZeroGuardSamples+1):(end-obj.Implementation.ZeroGuardSamples-obj.Implementation.OverlapSamplesTX)),obj.Implementation.TimeSpacing,obj.Nr.MCSymbols);
            
            ReceivedSignal_reshaped_RemoveTXWindowAndCP = ReceivedSignal_reshaped(1+obj.Implementation.OverlapSamplesTX+obj.Implementation.CyclicPrefix:end,:);

            WindowFunctionTotal     = [obj.Implementation.WindowingFunctionRX;ones(obj.Implementation.FFTSize-obj.Implementation.OverlapSamplesRX,1);obj.Implementation.WindowingFunctionRX(end:-1:1)];      
            ReceivedSignalWindowed  = bsxfun(@times,WindowFunctionTotal.^2,ReceivedSignal_reshaped_RemoveTXWindowAndCP);
            
            OverlapBegin = ReceivedSignalWindowed(end-ceil(obj.Implementation.OverlapSamplesRX/2)+1:end,:);
            OverlapEnd   = ReceivedSignalWindowed(1:floor(obj.Implementation.OverlapSamplesRX/2),:);
            OverLapTotal = [OverlapBegin;...
                            zeros(obj.Implementation.FFTSize-obj.Implementation.OverlapSamplesRX,obj.Nr.MCSymbols);...
                                OverlapEnd];

            ReceivedSignalWindowedOverlappedAdd = ReceivedSignalWindowed(size(OverlapEnd,1)+1:end-size(OverlapBegin,1),:)+OverLapTotal;
            
            ReceivedSignalWindowedOverlappedAddCircshift = circshift(ReceivedSignalWindowedOverlappedAdd,-size(OverlapBegin,1),1);
       
            ReceivedSymbolsTemp = fft(ReceivedSignalWindowedOverlappedAddCircshift);
            if obj.PHY.TransmitRealSignal
        ...
        function Pn = GetSymbolNoisePower(obj, Pn_time)
            ...
            WindowFunctionTotal     = [obj.Implementation.WindowingFunctionRX;ones(obj.Implementation.FFTSize-obj.Implementation.OverlapSamplesRX,1);obj.Implementation.WindowingFunctionRX(end:-1:1)];      
            WOLAFactor =  sum(WindowFunctionTotal.^4)/obj.Implementation.FFTSize; 
            Pn = (Pn_time*(obj.Nr.Subcarriers/(obj.Implementation.FFTSize)))*WOLAFactor;
        end
        ...
        function TimePos = GetTimeIndexMidPos(obj)
            ...
            TimePos = obj.Implementation.ZeroGuardSamples+obj.Implementation.CyclicPrefix+obj.Implementation.OverlapSamplesTX+round(obj.Implementation.OverlapSamplesRX/2)+round(obj.Implementation.FFTSize/2)+1+ (0:obj.Nr.MCSymbols-1)*obj.Implementation.TimeSpacing;
        end    
    end
end